...
def read_css_file(filename: str) -> str:
    path = Path(__file__).parent / "assets" / "css" / filename
    if not path.exists():
        return ""
...
DEFAULT_DATA = {
    ...
    "input_snapshots": [],  # 入力条件のスナップショット（顧客名/業態/検針票/台数/概算前提 等を丸ごと保存→呼び出し）
...
def _num0(v, default=0.0):
    """数量・単価などを安全に数値化。None/空/非数/NaN は default を返す。
    （`v or 0` は NaN が真値のため NaN を返してしまい int(NaN) で落ちる。その回避用）"""
    try:
        f = float(v)
...
# ══════════════════════════════════════════════════════════════════
# エクスポート ファイル名（規則：エクスポート年月日_企業名_種別.拡張子）
# ══════════════════════════════════════════════════════════════════
def _sanitize_name(s: str) -> str:
    """ファイル名に使えない文字・敬称・空白を除去。"""
    import re
    s = str(s or "").strip()
    for w in ("御中", "様", "株式会社", "（株）", "(株)"):
        s = s.replace(w, "")
    s = re.sub(r'[\\/:*?"<>|\r\n\t]', "", s)
...
def export_filename(client: str, kind: str, ext: str) -> str:
    """『エクスポート年月日_企業名_種別.拡張子』を生成。年月日は実行時の当日。"""
    from datetime import datetime
    ymd = datetime.now().strftime("%Y%m%d")
    comp = _sanitize_name(client)
    kind = _sanitize_name(kind) if kind else ""
    stem = f"{ymd}_{comp}" + (f"_{kind}" if kind else "")
    ext = str(ext or "").lstrip(".")
    return f"{stem}.{ext}"


# ══════════════════════════════════════════════════════════════════
# 入力条件のスナップショット（保存＝現在の入力一式 / 呼び出し＝session_stateへ復元）
# ══════════════════════════════════════════════════════════════════
# 復元対象の入力ウィジェットキー（いずれもキー付きに統一済み）。opt_* は分析オプションのトグル群。
SNAPSHOT_KEYS = [
    "in_client_name", "in_gyotai",
    "manual_ac_peak_pct", "manual_ac_kwh_pct",
    "gaisan_mode", "gaisan_total", "gaisan_ctrl",
    "g_unit_kw", "g_ctrl_ratio_pct", "g_ac_peak_pct",
    "g_material_pu", "g_setup_pu", "g_hw_fixed", "g_hw_pu",
    "man_total", "man_ctrl", "man_area", "man_material", "man_setup",
    "cost_detail_mode",
    "det_denzai", "det_mzu", "det_simx", "det_days", "det_area5",
    "det_trans", "det_lodging_md", "det_rentacar_days",
]
SNAPSHOT_PREFIXES = ("opt_",)


def collect_input_snapshot(ss) -> dict:
    """現在の入力一式（検針票＋各入力ウィジェット）をJSON化可能なdictにまとめる。"""
    snap = {"keys": {}}
    for k in SNAPSHOT_KEYS:
        if k in ss:
            snap["keys"][k] = ss[k]
    for k in list(ss.keys()):
        if any(str(k).startswith(p) for p in SNAPSHOT_PREFIXES):
            v = ss[k]
    ...
    for lk in ("model_rows", "sim_hw_rows"):
        v = ss.get(lk)
...
def apply_input_snapshot(ss, snap: dict) -> None:
    """スナップショットを session_state へ復元（呼び出し後は st.rerun() すること）。"""
    for k, v in (snap.get("keys") or {}).items():
        ss[k] = v
    dfd = snap.get("df_input")
    if isinstance(dfd, dict) and dfd.get("records") is not None:
        try:
            df = pd.DataFrame(dfd["records"])
            cols = dfd.get("columns")
            if cols:
                df = df.reindex(columns=cols)
            ss["df_input"] = df
        except Exception:
            pass
    # 機体は model_rows から再構築させるため units_df は解除
    ss["units_df"] = None
    ss["model_rows"] = snap.get("model_rows")
    ss["sim_hw_rows"] = snap.get("sim_hw_rows", [])
...
def estimate_units_from_meter(df: pd.DataFrame, app_data: dict, gyotai: str,
                              ac_override=None, overrides=None) -> dict:
    """機材情報が無い案件向けの「概算台数」推定。
    検針票の最大デマンド × 業態の空調ピーク比率 ÷ 標準室外機定格kW で室外機台数を概算する。
    overrides={"unit_kw":..,"ctrl_ratio":..,"ac_peak":..} で案件ごとに前提値を上書き可（未指定はマスタ既定）。
    ...
    if not (max_demand > 0):
        max_demand = 0.0
    if overrides.get("ac_peak") is not None:
        ac_peak = float(overrides["ac_peak"])
    elif ac_override and ac_override.get("ac_peak") is not None:
    ...
    unit_kw = float(overrides.get("unit_kw", _g.get("unit_kw", 10.0))) or 10.0
    ratio = float(overrides.get("ctrl_ratio", _g.get("ctrl_ratio", 0.9)))
...
def show_admin(app_data: dict) -> None:
    ...
    tab1, tab_mat, tab2, tab3, tab4, tab5, tab_img, tab_data = st.tabs([
        "計算ロジック変更",
        "機材単価マスタ（見積）",
        "馬力・機材テーブル",
        "業態マスタ設定",
    ...
    with tab2:
        ...
        if st.button("室外機1台あたりの機材セットを保存", key="save_per_unit_mat"):
            _rows = [{"商品名": str(r.get("商品名", "")).strip(),
                      "1台あたり数量": int(_num0(r.get("1台あたり数量")))}
    ...
    # ─── データ管理（バックアップ / 復元）────────────────
    with tab_data:
        st.subheader("データ管理（バックアップ / 復元）")
        st.caption("設定・マスタ・企業データ・保存案件・スナップショットはすべて `dps_app_data.json` に入っています。"
                   "ここから丸ごとバックアップ／復元できます。定期的なバックアップを推奨します。")

        # ── 全体バックアップ（DL）──
        st.markdown("##### ⬇️ 全体バックアップ（ダウンロード）")
        try:
            _backup_bytes = json.dumps(app_data, ensure_ascii=False, indent=2).encode("utf-8")
            _n_cases = len(app_data.get("saved_cases", []) or [])
            _n_snaps = len(app_data.get("input_snapshots", []) or [])
            _n_comp = len(app_data.get("company_power_records", {}) or {})
            st.caption(f"内容：保存案件 {_n_cases}件／スナップショット {_n_snaps}件／企業データ {_n_comp}社 ほか設定・マスタ一式")
            st.download_button("⬇️ バックアップをダウンロード", _backup_bytes,
                               file_name=export_filename("DPSデータ", "バックアップ", "json"),
                               mime="application/json", key="data_backup_dl", use_container_width=True)
        except Exception as _e:
            st.error(f"バックアップ生成でエラー：{_e}")

        st.divider()

        # ── 全体復元（アップロード）──
        st.markdown("##### ⬆️ 全体復元（バックアップから上書き）")
        st.warning("復元すると、現在の設定・マスタ・保存案件・企業データ・スナップショットは"
                   "**アップロードした内容で完全に置き換わります**（元に戻せません）。先に上のバックアップDLを推奨。")
        _rf = st.file_uploader("バックアップ（.json）をアップロード", type=["json"], key="data_restore_up")
        if _rf is not None:
            try:
                _rf.seek(0)
                _loaded = json.loads(_rf.read().decode("utf-8"))
            except Exception as _e:
                _loaded = None
                st.error(f"読み込めませんでした（JSON形式をご確認ください）：{_e}")
            if isinstance(_loaded, dict):
                _keys_ok = any(k in _loaded for k in ("const_settings", "gyotai_master", "saved_cases"))
                if not _keys_ok:
                    st.error("このファイルは本アプリのバックアップ形式ではないようです（想定キーが見つかりません）。")
                else:
                    st.info(f"復元候補：保存案件 {len(_loaded.get('saved_cases', []) or [])}件／"
                            f"スナップショット {len(_loaded.get('input_snapshots', []) or [])}件／"
                            f"企業データ {len(_loaded.get('company_power_records', {}) or {})}社")
                    _confirm = st.checkbox("内容を確認しました。現在のデータを上書きして復元します。", key="data_restore_ok")
                    if st.button("⬆️ このバックアップで完全復元", key="data_restore_btn",
                                 type="primary", disabled=not _confirm):
                        save_data(_loaded)
                        st.success("復元しました。最新の内容で再読み込みします。")
                        st.rerun()

        st.divider()

        # ── スナップショット単体の書き出し / 取り込み ──
        st.markdown("##### 🔁 スナップショット単体の受け渡し（別PC・他担当へ）")
        st.caption("入力条件のスナップショットを1件だけ .json で書き出し／取り込みできます。"
                   "JSON全体を共有せずに、案件の入力条件だけを渡せます。")
        _snaps = app_data.setdefault("input_snapshots", [])
        if _snaps:
            _slabels = [f"{s.get('name', '(無題)')}｜{s.get('client', '')}｜{s.get('saved_at', '')}" for s in _snaps]
            _sidx = st.selectbox("書き出すスナップショット", list(range(len(_snaps))),
                                 format_func=lambda i: _slabels[i], key="snap_export_pick")
            _one = _snaps[_sidx]
            _one_bytes = json.dumps(_one, ensure_ascii=False, indent=2).encode("utf-8")
            st.download_button("⬇️ このスナップショットを書き出し（.json）", _one_bytes,
                               file_name=export_filename(_one.get("client", "") or _one.get("name", "snapshot"),
                                                         "入力条件", "json"),
                               mime="application/json", key="snap_export_dl", use_container_width=True)
        else:
            st.caption("スナップショットがまだありません（シミュレーション画面で保存できます）。")

        _sf = st.file_uploader("スナップショット（.json）を取り込み", type=["json"], key="snap_import_up")
        if _sf is not None:
            try:
                _sf.seek(0)
                _sone = json.loads(_sf.read().decode("utf-8"))
            except Exception as _e:
                _sone = None
                st.error(f"読み込めませんでした：{_e}")
            if isinstance(_sone, dict) and isinstance(_sone.get("snapshot"), dict):
                st.info(f"取込候補：{_sone.get('name', '(無題)')}｜{_sone.get('client', '')}")
                if st.button("⬆️ このスナップショットを追加", key="snap_import_btn"):
                    _nm = _sone.get("name", "取込") or "取込"
                    _snaps[:] = [s for s in _snaps if s.get("name") != _nm]  # 同名は上書き
                    _snaps.append(_sone)
                    app_data["input_snapshots"] = _snaps
                    save_data(app_data)
                    st.success(f"「{_nm}」を取り込みました。シミュレーション画面の呼び出しで使えます。")
            elif _sone is not None:
                st.error("スナップショット形式ではないようです（`snapshot` キーが必要）。")
...
def show_results_list(app_data: dict) -> None:
    ...
    # ── 🔎 検索（企業名・業態でしぼり込み）──
    _q = st.text_input("🔎 検索（企業名・業態）", key="list_search",
                       placeholder="企業名や業態の一部を入力（空欄で全件）").strip().lower()

    def _match(c):
        if not _q:
            return True
        return (_q in str(c.get("company", "")).lower()
                or _q in str(c.get("gyotai", "")).lower())

    # (元インデックス, case) のペアで保持し、削除時に元の位置へ正しく反映
    view = [(i, c) for i, c in enumerate(cases) if _match(c)]

    if _q:
        st.caption(f"該当：{len(view)} 件 / 全 {len(cases)} 件")
    else:
        st.caption(f"保存件数：{len(cases)} 件（詳細データは各社フォルダで管理。ここは一覧用の軽量サマリーです）")

    if not view:
        st.info("検索条件に一致する案件がありません。キーワードを変えてください。")
        return
    ...
    # ── 🆚 並べて比較（2〜3件）──
    st.markdown("##### 🆚 案件を並べて比較（2〜3件）")
    _cmp_labels = [f"{oi+1}. {c.get('company','—')}（{c.get('saved_at','')}）" for oi, c in view]
    _cmp_map = {lab: view[j][1] for j, lab in enumerate(_cmp_labels)}
    _cmp_sel = st.multiselect("比較する案件を選択（最大3件）", _cmp_labels, key="list_cmp_sel",
                              max_selections=3)
    if len(_cmp_sel) >= 2:
        def _yen(v):
            try:
        ...
        def _kwh(v):
            try:
                return f"{float(v):,.0f} kWh"
            except Exception:
                return "—"
        _metric_rows = ["業態", "空調台数", "初期費用", "年間削減量", "年間削減額", "回収年数", "保存日時"]
        _cmp = {"項目": _metric_rows}
        for lab in _cmp_sel:
            c = _cmp_map[lab]
            _cmp[c.get("company", "—")] = [
                c.get("gyotai", "—"),
                f"{int(c.get('units_ctrl', 0))} 台",
                _yen(c.get("invest")) if c.get("invest") is not None else "—（旧保存）",
                _kwh(c.get("saving_kwh", 0)),
                _yen(c.get("saving_yen", 0)),
                _pb(c.get("payback")),
                c.get("saved_at", "—"),
            ]
        st.dataframe(pd.DataFrame(_cmp), use_container_width=True, hide_index=True)
        st.caption("※ 初期費用は本バージョン以降に保存した案件のみ表示されます（旧案件は「—（旧保存）」）。")
    elif len(_cmp_sel) == 1:
        st.caption("あと1件以上選ぶと比較表を表示します。")

    # ── 削除 ──（検索結果の中から選択。元の位置へ反映）
    st.markdown("##### 🗑️ 案件の削除")
    labels = [f"{oi+1}. {c.get('company','—')}（{c.get('saved_at','')}）" for oi, c in view]
    sel = st.selectbox("削除する案件を選択", ["（選択してください）"] + labels, key="list_del_sel")
    if st.button("🗑️ 選択した案件を削除", key="list_del_btn"):
        if sel in labels:
            _orig_idx = view[labels.index(sel)][0]
            cases.pop(_orig_idx)
...
def show_calc_methodology(res: dict) -> None:
    ...
    if hardware_cost > 0 or _hw_rows_m:
        _hw_apply = "　＋　".join(
            f"{r.get('商品名','')} {yen(_num0(r.get('売価単価')))}×{int(_num0(r.get('数量')))}"
            for r in _hw_rows_m if int(_num0(r.get('数量'))) > 0
        ) or "（数量0）"
        html += card(
...
def show_quote_builder(app_data: dict, res: dict, client_name: str) -> None:
    ...
    mat_edit = st.data_editor(
        pd.DataFrame(mat_rows), num_rows="dynamic", use_container_width=True,
        # key は付けない（付けると削除/追加の編集状態が残り、書き戻したデータと二重適用され行削除でエラー）
        column_config={
            "商品名": st.column_config.TextColumn("商品名", width="large"),
            ...
            if st.session_state.get("_dash_ppt"):
                try:
                    st.download_button("PPTXダウンロード", build_pptx(res, client_name, app_data),
                                       file_name=export_filename(client_name, "提案書", "pptx"), use_container_width=True, key="dash_ppt_dl")
                except Exception as _e:
            ...
            if st.session_state.get("_dash_xls"):
                try:
                    st.download_button("XLSXダウンロード", build_excel(res, client_name, app_data),
                                       file_name=export_filename(client_name, "試算", "xlsx"), use_container_width=True, key="dash_xls_dl")
                except Exception as _e:
                    st.caption(f"生成エラー：{_e}")
        with _o[2]:
        ...
        if not HAS_SLIDE_IMG:
            ...
            if st.session_state.get("_img_chart"):
                ...
                st.download_button("グラフPNGをダウンロード", st.session_state["_img_chart"],
                                   file_name=export_filename(client_name, "使用量デマンドグラフ", "png"), mime="image/png",
                                   key="dl_img_chart")
            ...
            if _list_imgs:
                ...
                for _k, _img in enumerate(_list_imgs, start=1):
                    st.image(_img, use_container_width=True)
                    _kind = "制御可否リスト" if _n_pg == 1 else f"制御可否リスト_{_k}of{_n_pg}"
                    _fname = export_filename(client_name, _kind, "png")
            ...
            if st.session_state.get("_img_cards"):
                ...
                st.download_button("サマリカードPNGをダウンロード", st.session_state["_img_cards"],
                                   file_name=export_filename(client_name, "サマリカード", "png"), mime="image/png",
                                   key="dl_img_cards")
    ...
    def _make_output():
        ...
        if fmt == "Excel":
            return ("Excelをダウンロード", build_reduction_report_excel(R),
                    export_filename(safe, "削減レポート", "xlsx"),
        ...
        return ("PDFをダウンロード", build_reduction_report_pdf(R),
                export_filename(safe, "削減レポート", "pdf"), "application/pdf", "rep_dl_pdf")
        ...
        try:
            if ifmt == "PDF":
                st.download_button("請求書（PDF）をダウンロード", build_invoice_pdf(INV),
                                   file_name=export_filename(client or "請求書", "請求書", "pdf"),
                                   mime="application/pdf", key="inv_dl_pdf")
            else:
                st.download_button("請求書（Excel）をダウンロード", build_invoice_excel(INV),
                                   file_name=export_filename(client or "請求書", "請求書", "xlsx"),
                                   mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                   key="inv_dl_xlsx")
        except Exception as _e:
...
""", unsafe_allow_html=True)
        ...
        # ── 💾 入力条件のスナップショット（保存 / 呼び出し）──
        #   ※ 復元は下の入力ウィジェットが作られる前に実行する必要があるため、ここに配置。
        with st.expander("💾 入力条件のスナップショット（保存 / 呼び出し）", expanded=False):
            st.caption("現在の入力一式（顧客名・業態・検針票・台数・概算前提・初期費用設定など）を1件として保存し、"
                       "後日や別PCで呼び出して同じ条件を再現できます。※各種設定のマスタや保存案件とは別枠です。")
            _snaps = app_data.setdefault("input_snapshots", [])
            _sc1, _sc2 = st.columns([3, 1])
            _snap_name = _sc1.text_input("スナップショット名", key="snap_name_in",
                                         placeholder="例：ゼンケン二本松_概算v1")
            if _sc2.button("💾 保存", key="snap_save_btn", use_container_width=True):
                _nm = (_snap_name or "").strip() or (st.session_state.get("in_client_name", "") or "無題")
                _rec = {"name": _nm,
                        "client": st.session_state.get("in_client_name", ""),
                        "gyotai": st.session_state.get("in_gyotai", ""),
                        "saved_at": datetime.datetime.now().strftime("%Y-%m-%d %H:%M"),
                        "snapshot": collect_input_snapshot(st.session_state)}
                _snaps[:] = [s for s in _snaps if s.get("name") != _nm]  # 同名は上書き
                _snaps.append(_rec)
                app_data["input_snapshots"] = _snaps
                save_data(app_data)
                st.success(f"「{_nm}」を保存しました。")
            if _snaps:
                _labels = [f"{s.get('name', '(無題)')}｜{s.get('client', '')}｜{s.get('saved_at', '')}" for s in _snaps]
                _idx = st.selectbox("呼び出すスナップショット", list(range(len(_snaps))),
                                    format_func=lambda i: _labels[i], key="snap_pick")
                _lc1, _lc2 = st.columns(2)
                if _lc1.button("📥 呼び出して復元", key="snap_load_btn", use_container_width=True):
                    apply_input_snapshot(st.session_state, _snaps[_idx].get("snapshot", {}))
                    st.success(f"「{_snaps[_idx].get('name')}」を復元しました。")
                    st.rerun()
                if _lc2.button("🗑️ このスナップショットを削除", key="snap_del_btn", use_container_width=True):
                    _snaps.pop(_idx)
                    app_data["input_snapshots"] = _snaps
                    save_data(app_data)
                    st.rerun()
            else:
                st.caption("まだスナップショットはありません。入力後に『保存』してください。")

        # ── 顧客情報 ─────────────────────────────
        st.markdown("### 顧客情報")
        st.session_state.setdefault("in_client_name", "")
        client_name = st.text_input("顧客名", key="in_client_name")
        MANUAL_GYOTAI = "その他※数値を指定する"
        gyotai_list = list(app_data["gyotai_master"].keys()) + [MANUAL_GYOTAI]
        _gy_def = gyotai_list[1] if len(gyotai_list) > 1 else gyotai_list[0]
        st.session_state.setdefault("in_gyotai", _gy_def)
        if st.session_state.get("in_gyotai") not in gyotai_list:
            st.session_state["in_gyotai"] = _gy_def
        gyotai = st.selectbox("業態", gyotai_list, key="in_gyotai")
        ...
        if units_df is not None and len(units_df) > 0:
            ...
            if _gaisan:
                ...
                _gsx = app_data.get("const_settings", {}).get("gaisan_settings", {})
                # ── 概算の前提（この案件用に調整）。既定はマスタ値を反映し、自由に上書き可 ──
                with st.expander("🔧 概算の前提（この案件用に調整）", expanded=False):
                    st.caption("台数の推定根拠や標準単価を、この案件だけ手動で調整できます（既定はマスタ値）。"
                               "空欄に戻したいときは各種設定のマスタ値を目安にしてください。")
                    st.session_state.setdefault("g_unit_kw", float(_gsx.get("unit_kw", 10.0)) or 10.0)
                    st.session_state.setdefault("g_ctrl_ratio_pct", int(round(float(_gsx.get("ctrl_ratio", 0.9)) * 100)))
                    st.session_state.setdefault("g_material_pu", int(float(_gsx.get("material_per_unit", 100000))))
                    st.session_state.setdefault("g_setup_pu", int(float(_gsx.get("setup_per_unit", 50000))))
                    st.session_state.setdefault("g_hw_fixed", int(float(_gsx.get("hw_fixed", 300000))))
                    st.session_state.setdefault("g_hw_pu", int(float(_gsx.get("hw_per_unit", 50000))))
                    _go1, _go2 = st.columns(2)
                    with _go1:
                        st.number_input("標準室外機 定格出力（kW/台）", min_value=0.5, max_value=200.0,
                                        step=0.5, key="g_unit_kw",
                                        help="概算台数 ＝ 空調ピーク負荷 ÷ この定格出力。大きいほど台数は少なく出ます。")
                        st.number_input("概算 制御比率（％）", min_value=1, max_value=100, step=1,
                                        key="g_ctrl_ratio_pct",
                                        help="総台数のうち制御対象とする割合。制御台数の初期値に使用。")
                        st.number_input("空調ピーク比率の上書き（％／0=業態マスタ）", min_value=0, max_value=100,
                                        step=1, key="g_ac_peak_pct",
                                        help="0なら業態マスタの空調ピーク比率を使用。手元に根拠があれば上書き可。")
                    with _go2:
                        st.number_input("材料費（円/台）", min_value=0, max_value=100000000, step=10000, key="g_material_pu")
                        st.number_input("工事費（円/台）", min_value=0, max_value=100000000, step=10000, key="g_setup_pu")
                        st.number_input("ハードウェア固定費（円・拠点1式）", min_value=0, max_value=100000000,
                                        step=10000, key="g_hw_fixed")
                        st.number_input("ハードウェア費（円/台）", min_value=0, max_value=100000000, step=10000, key="g_hw_pu")
                _g_over = {
                    "unit_kw": float(st.session_state.get("g_unit_kw", 10.0)),
                    "ctrl_ratio": float(st.session_state.get("g_ctrl_ratio_pct", 90)) / 100.0,
                }
                if int(st.session_state.get("g_ac_peak_pct", 0)) > 0:
                    _g_over["ac_peak"] = float(st.session_state["g_ac_peak_pct"]) / 100.0
                _estu = estimate_units_from_meter(st.session_state.df_input, app_data, gyotai,
                                                  ac_override=_ac_ovr_g, overrides=_g_over)
                st.info(f"概算台数：空調ピーク {_estu['ac_peak_kw']:.0f}kW ÷ 標準室外機 {_estu['unit_kw']:.1f}kW"
                ...
                st.session_state.setdefault("man_total", 34)
                n_units_total = st.number_input("室外機 総台数", min_value=1, max_value=5000, key="man_total")
                if int(st.session_state.get("man_ctrl", 31)) > int(n_units_total):
                    st.session_state["man_ctrl"] = int(n_units_total)
                st.session_state.setdefault("man_ctrl", min(31, int(n_units_total)))
                n_units_ctrl  = st.number_input("うち制御対象台数", min_value=1,
                                                 max_value=int(n_units_total), key="man_ctrl")
        ...
        if not detail_mode:
            ...
            _area_keys = list(AREA_BUFFER.keys())
            _area_def = _area_keys[1] if len(_area_keys) > 1 else _area_keys[0]
            st.session_state.setdefault("man_area", _area_def)
            if st.session_state.get("man_area") not in _area_keys:
                st.session_state["man_area"] = _area_def
            area = st.selectbox("施工場所エリア", _area_keys, key="man_area")
            ...
            if st.session_state.get("gaisan_mode"):
                ...
                # 案件ごとの前提（概算パネル）を優先。未設定時はマスタ既定。
                _mpu = float(st.session_state.get("g_material_pu", _gs.get("material_per_unit", 100000)))
                _spu = float(st.session_state.get("g_setup_pu", _gs.get("setup_per_unit", 50000)))
                system_cost = int(round(n_units_ctrl * _mpu))
                setup_cost  = int(round(n_units_ctrl * _spu))
                st.metric("材料費（概算：単価×台数）", f"¥{system_cost:,}")
                st.caption(f"材料費 ¥{_mpu:,.0f}/台 × 制御 {n_units_ctrl}台（『概算の前提』で調整可）")
                st.metric("工事費（概算：単価×台数）", f"¥{setup_cost:,}")
                st.caption(f"工事費 ¥{_spu:,.0f}/台 × 制御 {n_units_ctrl}台（『概算の前提』で調整可）")
            else:
                st.session_state.setdefault("man_material", 0)
                st.session_state.setdefault("man_setup", 0)
                system_cost  = st.number_input("材料費（円）", min_value=0,
                                                max_value=100000000, step=100000, key="man_material")
                setup_cost   = st.number_input("工事費（円）", min_value=0,
                                                max_value=10000000, step=50000, key="man_setup")
        else:
            # ── 詳細見積（見積書方式）──
        ...
        if st.session_state.get("gaisan_mode") and not hw_rows:
            ...
            _hwf = float(st.session_state.get("g_hw_fixed", _gs2.get("hw_fixed", 300000)))
            _hwu = float(st.session_state.get("g_hw_pu", _gs2.get("hw_per_unit", 50000)))
            hardware_cost = int(round(_hwf + n_units_ctrl * _hwu))
            st.metric("ハードウェア関連費（概算：固定＋単価×台数）", f"¥{hardware_cost:,}")
            st.caption(f"固定 ¥{_hwf:,.0f}（拠点1式：データ収集装置/親機等） ＋ ¥{_hwu:,.0f}/台 × 制御 {n_units_ctrl}台"
                       "（『概算の前提』で調整可）")
        else:
            hardware_cost = int(sum(_num0(r.get("売価単価")) * int(_num0(r.get("数量"))) for r in hw_rows))
        ...
        _kq_temp = int(sum(int(_num0(r.get("数量")))                          # 温湿度センサー＝防水温湿度計の個数
        ...
        # 本業商品設定（主力商品単価・粗利益率）の入力項目は、本業売上換算スライドの削除に伴い撤去。
        # 値は各種設定のマスタ（item_price / item_margin）をそのまま使用する（Excel等の換算表記は従来どおり）。
        item_price  = int(app_data.get("item_price", 5000))
        item_margin = float(app_data.get("item_margin", 0.30))
...
""", unsafe_allow_html=True)
    ...
    with st.expander("機体（室外機）入力 — 型番から馬力を自動反映（管理外は仮置き）", expanded=False):
        ...
        # 列順を固定（機器ID/場所 の次に 階/エリア）＋ 欠損列を補完
        _order = ["機器ID/場所", "階/エリア", "メーカー", "型番", "稼働係数", "制御可否"]
        for _c in _order:
            if _c not in model_in.columns:
                model_in[_c] = ("○" if _c == "制御可否" else (0.7 if _c == "稼働係数" else ""))
        model_in = model_in[_order + [c for c in model_in.columns if c not in _order]]

        # ── dtype を明示（空列が float 推定→TextColumn 非互換になるのを防ぐ）──
        def _as_text(v):
            if v is None or (isinstance(v, float) and pd.isna(v)):
                return ""
            return str(v)
        for _tc in ("機器ID/場所", "階/エリア", "メーカー", "型番"):
            model_in[_tc] = model_in[_tc].map(_as_text).astype("object")
        model_in["制御可否"] = model_in["制御可否"].map(
            lambda v: "×" if str(v).strip() in ("×", "x", "X", "0", "False", "false", "不可") else "○").astype("object")
        model_in["稼働係数"] = pd.to_numeric(model_in["稼働係数"], errors="coerce").fillna(0.7).astype(float)
        edited_models = st.data_editor(
            ...
            # ※ key は付けない：キーを付けると削除/追加の編集状態が残り、
            #   書き戻した model_rows と二重適用されて行削除でエラーになるため（入力ドリブンで反映）。
        )
    ...
    with st.expander("ハードウェア関連（部材）一覧 — 取り込んだ部材を機材単価マスタの単価で自動反映（編集可）",
        ...
        _hw_total = int(sum(_num0(r.get("売価単価")) * int(_num0(r.get("数量")))
                            for r in _hw_rows2))
    ...
    if not HAS_SLIDE_IMG:
        ...
        if list_imgs:
            ...
            for k, img in enumerate(list_imgs, start=1):
                ...
                _kind = "制御可否リスト" if n_pg == 1 else f"制御可否リスト_{k}of{n_pg}"
                fname = export_filename(client_name, _kind, "png")
...
""", unsafe_allow_html=True)
    ...
    with col_dl1:
        ...
        filename_xl = export_filename(client_name, "試算", "xlsx")
    ...
    with col_dl2:
        ...
        if HAS_PPTX:
            filename_pp = export_filename(client_name, "提案書", "pptx")
