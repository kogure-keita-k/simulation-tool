...
"""
import sys, calendar, os, copy, statistics
...
def build(inp,out):
    ...
    sys1=f(cfg.get("システム一式") or cfg.get("AIシステム一式"),1400000)  # 固定・機材
    setupc=f(cfg.get("セットアップ費"),400000)                            # 固定
    koji=f(cfg.get("工事費単価") or cfg.get("工事費"),50000)             # 円/台
    areaf=f(cfg.get("エリア係数"),1.00)                                  # 工事費の地域係数
    fee=f(cfg.get("年間利用料単価") or cfg.get("利用料単価"),12000)      # 円/台・年
    hojo=f(cfg.get("補助金"),0)
    area=cfg.get("エリア") or "関東"
    ratiofix=f(cfg.get("制御可能比率固定値"),1.00); coincdef=f(cfg.get("稼働係数既定"),0.85)
    contractfix=f(cfg.get("契約電力固定値"),0)
    ...
    # 考察シート用の追加指標（Python側で算出）
    _hours=sum(dy)*24 or 1
    _avg_kw=sum(kwh)/_hours
    _lf=(_avg_kw/max(pk)) if (pk and max(pk)) else 0
    _cv=(statistics.pstdev(kwh)/(sum(kwh)/len(kwh))) if kwh else 0
    _ene=sum(k*r for k,r in zip(kwh,rate)); _bas=sum(b*c for b,c in zip(bu,ct))
    _basic_ratio=_bas/(_ene+_bas) if (_ene+_bas) else 0
    _eff_rate=(_ene+_bas)/sum(kwh) if sum(kwh) else 0
    _valley_summer=str(kwmin[0]).replace("-","/").split("/")[-1] in {"06","07","08","09"}
    ...
    PF="*((185-'前提・制御条件'!$B$10)/100)"  # 力率割引係数（力率100%→0.85）
    SYS="'前提・制御条件'!$B$23";SETUP="'前提・制御条件'!$B$24";KOJI="'前提・制御条件'!$B$25";AREAF="'前提・制御条件'!$B$26";FEE="'前提・制御条件'!$B$27";HOJO="'前提・制御条件'!$B$28"  # 経済性マスタ
    RMODE="'前提・制御条件'!$B$31";RFIX="'前提・制御条件'!$B$32";CMODE="'前提・制御条件'!$B$33";CFIX="'前提・制御条件'!$B$34"  # フォールバック
    # 前提
    ...
    st(s.cell(10,1,"契約力率(%)"),BLACK,align=LEFT);st(s.cell(10,2,100),BLUE,YEL,CEN,"0")
    st(s.cell(10,4,"力率100%→基本料金×0.85。検針票のG単価が割引前menu単価のとき適用。85入力で実質無効化。"),Font(name=Fn,size=9,color="595959"),align=LEFT)
    ...
    s["A22"]="【4】経済性の前提（初期費用・年間費／黄=編集可・全シート連動）"; s["A22"].font=BOLD
    st(s.cell(23,1,"システム一式（機材・固定）"),BLACK,align=LEFT);st(s.cell(23,2,sys1),BLUE,YEL,CEN,"#,##0");st(s.cell(23,3,"円"),BLACK,align=CEN)
    st(s.cell(24,1,"セットアップ費（固定）"),BLACK,align=LEFT);st(s.cell(24,2,setupc),BLUE,YEL,CEN,"#,##0");st(s.cell(24,3,"円"),BLACK,align=CEN)
    st(s.cell(25,1,"工事費 台単価"),BLACK,align=LEFT);st(s.cell(25,2,koji),BLUE,YEL,CEN,"#,##0");st(s.cell(25,3,"円/台"),BLACK,align=CEN)
    st(s.cell(26,1,"エリア係数（工事費）"),BLACK,align=LEFT);st(s.cell(26,2,areaf),BLUE,YEL,CEN,"0.00")
    st(s.cell(26,4,f"想定:{area}。関東1.00/中部北陸1.05/近畿1.02/北海道東北1.10/中国四国1.08/九州沖縄1.12"),Font(name=Fn,size=9,color="595959"),align=LEFT)
    st(s.cell(27,1,"年間サービス利用料"),BLACK,align=LEFT);st(s.cell(27,2,fee),BLUE,YEL,CEN,"#,##0");st(s.cell(27,3,"円/台・年"),BLACK,align=CEN)
    st(s.cell(27,4,"毎年発生→純メリットから自動控除"),Font(name=Fn,size=9,color="C00000"),align=LEFT)
    st(s.cell(28,1,"補助金・助成金（−）"),BLACK,align=LEFT);st(s.cell(28,2,hojo),BLUE,YEL,CEN,"#,##0");st(s.cell(28,3,"円"),BLACK,align=CEN)
    s["A30"]="【5】データ無し時のフォールバック（固定値を選択可・運用配慮）"; s["A30"].font=BOLD
    st(s.cell(31,1,"制御可能比率の取得"),BLACK,align=LEFT);st(s.cell(31,2,"台帳から自動"),BLUE,YEL,CEN)
    st(s.cell(31,4,"機器台帳が無い/不完全なら「固定値」と入力（下の固定値を使用）"),Font(name=Fn,size=9,color="595959"),align=LEFT)
    st(s.cell(32,1,"　└ 制御可能比率 固定値"),BLACK,align=LEFT);st(s.cell(32,2,ratiofix),BLUE,YEL,CEN,"0%")
    st(s.cell(33,1,"契約電力の取得"),BLACK,align=LEFT);st(s.cell(33,2,"検針票から"),BLUE,YEL,CEN)
    st(s.cell(33,4,"検針票に契約電力が無いなら「固定値」と入力（固定値0なら年間最大デマンドを使用）"),Font(name=Fn,size=9,color="595959"),align=LEFT)
    st(s.cell(34,1,"　└ 契約電力 固定値(kW)"),BLACK,align=LEFT);st(s.cell(34,2,contractfix),BLUE,YEL,CEN,"0")
    st(s.cell(35,1,"稼働係数 既定値（台帳 未記入機）"),BLACK,align=LEFT);st(s.cell(35,2,coincdef),BLUE,YEL,CEN,"0.00")
    for col,w in {"A":34,"B":13,"C":9,"D":50}.items(): s.column_dimensions[col].width=w
    # 月次
    ...
    for i in range(n):
        ...
        st(m.cell(r,9,f"=C{r}*F{r}"),BLACK,None,None,"#,##0");st(m.cell(r,10,f"=G{r}*H{r}"+PF),BLACK,None,None,"#,##0")
    tr=4+n
    ...
    for idx,(lc,mk,md,kh,rt) in enumerate(eq,1):
        ...
        st(eqs.cell(r,7,(rt if kh=="〇" and rt!="" else None)),BLACK,fill,CEN,"0.0")
        st(eqs.cell(r,8,coinc(rt)),BLUE,YEL,CEN,"0.00");st(eqs.cell(r,9,f'=IF(F{r}="","",F{r}*H{r})'),BLACK,fill,CEN,"0.0");st(eqs.cell(r,10,f'=IF(AND(E{r}="〇",F{r}<>""),I{r},0)'),BLACK,fill,CEN,"0.0");r+=1
    tot=r
    ...
    st(eqs.cell(tot,9,f"=SUM(I5:I{r-1})"),WB_,HDR,CEN,"0.0");st(eqs.cell(tot,10,f"=SUM(J5:J{r-1})"),WB_,HDR,CEN,"0.0")
    sb=tot+2
    ...
    st(eqs.cell(sb+2,2,"制御可能 比率(容量×稼働 加重)"),BOLD,YEL,LEFT);st(eqs.cell(sb+2,6,f"=IF(I{tot}=0,0,J{tot}/I{tot})"),BLACK,YEL,CEN,"0.0%")
    st(eqs.cell(sb+3,2,f"物理台数：{len(eq)}台（制御可{sum(1 for e in eq if e[3]=='〇')}／不可{sum(1 for e in eq if e[3]!='〇')}）"),RED,align=LEFT)
    RATIO=f"機器台帳!$F${sb+2}";WSUM=f"機器台帳!$I${tot}"
    for col,w in {"A":4,"B":18,"C":10,"D":22,"E":7,"F":10,"G":10,"H":8,"I":11,"J":10}.items(): eqs.column_dimensions[col].width=w
    ...
    ("空調 ピーク寄与kW","=B5*B6","kW","#,##0","b"),("制御可能比率",f"=IF(OR({RMODE}=\"固定値\",{WSUM}=0),{RFIX},{RATIO})","","0.0%","g"),
    ...
    OLDC=f"IF({CMODE}=\"固定値\",IF({CFIX}>0,{CFIX},$B$5),IF(MAX(月次・電気料金!H4:H{tr-1})>0,MAX(月次・電気料金!H4:H{tr-1}),$B$5))"
    MIND=f"MIN(月次・電気料金!D4:D{tr-1})"
    B1="MIN($B$9*{cap},("+OLDC+")-("+MIND+"))*$B$13*12"+PF   # ① 基本料金削減（実契約−最小需要の床／実量制）
    defs=[("定格キャップ率","={cap}","0%",0),("デマンド削減 ΔkW","=$B$9*{cap}","#,##0",0),("制御後 最大デマンド","=$B$5-$B$9*{cap}","#,##0",0),
    ("① 基本料金 削減/年","="+B1,"#,##0",1),("空調kWh 削減/年","=$B$11*{cap}*"+CONV,"#,##0",0),
    ("② 従量料金 削減/年","=$B$11*{cap}*"+CONV+"*$B$12","#,##0",1),
    ("合計 削減/年 (①+②)","="+B1+"+$B$11*{cap}*"+CONV+"*$B$12","#,##0",1),
    ("対 年間電気代 削減率","=("+B1+"+$B$11*{cap}*"+CONV+"*$B$12)/($B$14+$B$15)","0.0%",1)]
    SCEN_ROW=26
    ...
    st(ro.cell(4,1,"1台あたり工事費(ランキング用)"),BLACK,align=LEFT);st(ro.cell(4,2,f"={KOJI}"),GREEN,None,CEN,"#,##0");st(ro.cell(4,3,"円/台(前提連動)"),BLACK,align=CEN)
    st(ro.cell(5,1,"適用キャップ率"),BLACK,align=LEFT);st(ro.cell(5,2,"='前提・制御条件'!$B$14"),BLACK,YEL,CEN,"0%")
    ...
    st(ro.cell(7,2,"='空調制御シナリオ'!$C$26"),GREEN,None,CEN,"#,##0")
    ...
    # ============ 年間利用料・回収（経済性） ============
    yr=wb.create_sheet("年間利用料・回収"); yr.sheet_view.showGridLines=False
    yr["A1"]="年間利用料・投資回収（初期費用＋年間費 込み）"; yr["A1"].font=TITLE
    yr["A2"]="台数・単価は前提シート【4】のマスタに連動。利用料は毎年の削減から控除し、純メリット・回収年を算定。"; yr["A2"].font=Font(name=Fn,italic=True,color="C00000")
    ROIG5="'室外機絞り込みROI'!$G$5"; ROIK4="'室外機絞り込みROI'!$K$4"
    erows=[
     ("採用台数（回収しきい値クリア）", f"={ROIG5}", "台", "0", False),
     ("　機材費 ＝ システム一式 ＋ セットアップ（固定）", f"={SYS}+{SETUP}", "円", "#,##0", False),
     ("　工事費 ＝ 台数 × 台単価 × エリア係数", f"=B3*{KOJI}*{AREAF}", "円", "#,##0", False),
     ("　補助金（−）", f"={HOJO}", "円", "#,##0", False),
     ("初期費用 合計（A）＝ 機材費＋工事費−補助金", "=B4+B5-B6", "円", "#,##0", True),
     ("", "", "", "", False),
     ("年間 グロス削減額", f"={ROIK4}", "円/年", "#,##0", False),
     ("年間 サービス利用料 ＝ 台数 × 利用料単価", f"=B3*{FEE}", "円/年", "#,##0", False),
     ("年間 純メリット（B）＝ 削減 − 利用料 ★", "=B9-B10", "円/年", "#,##0", True),
     ("単純投資回収年 ＝ A ÷ B", "=IF(B11<=0,99,B7/B11)", "年", "0.0", True),
     ("当社 年間ストック収益（利用料）", "=B10", "円/年", "#,##0", False),
    ]
    r=3
    for lab,ff,u,fmt,hi in erows:
        if lab=="": r+=1; continue
        st(yr.cell(r,1,lab),BOLD if hi else BLACK,GREENF if hi else None,LEFT)
        st(yr.cell(r,2,ff),GRN if hi else GREEN,GREENF if hi else None,CEN,fmt)
        st(yr.cell(r,3,u),BLACK,GREENF if hi else None,CEN); r+=1
    yr["A15"]="▶ 累積収支シミュレーション（純メリットベース・利用料控除後）"; yr["A15"].font=BOLD
    st(yr.cell(16,1,"経過年"),WB_,HDR,CEN)
    st(yr.cell(17,1,"累積収支(円)"),BOLD,align=LEFT)
    for k in range(1,16):
        st(yr.cell(16,1+k,f"{k}年"),WB_,HDR,CEN)
        st(yr.cell(17,1+k,f"=$B$11*{k}-$B$7"),BLACK,None,CEN,"#,##0")
    yr["A19"]="※ 年間サービス利用料は毎年発生する費用として純メリットから必ず控除済み。初期費用＝機材費＋工事費−補助金（前提シート【4】で一括管理）。"; yr["A19"].font=Font(name=Fn,size=9,color="595959")
    yr.column_dimensions["A"].width=40
    for k in range(2,17): yr.column_dimensions[get_column_letter(k)].width=12
    # ============ 考察・追加分析 ============
    an=wb.create_sheet("考察・追加分析"); an.sheet_view.showGridLines=False
    an["A1"]="考察・追加分析（他シートから自動算出）"; an["A1"].font=TITLE
    an["A2"]="月次・シナリオ・機器台帳の数値から自動算出。前提値依存の指標は感応度欄を併読。"; an["A2"].font=Font(name=Fn,italic=True,color="C00000")
    def MN(c): return f"'月次・電気料金'!{c}"
    def SC(c): return f"'空調制御シナリオ'!{c}"
    # ① 負荷特性の定量指標
    an["A4"]="① 負荷特性の定量指標"; an["A4"].font=BOLD
    for j,h in enumerate(["指標","値","単位","評価・意味"],1): st(an.cell(5,j,h),WB_,HDR,CEN)
    rows1=[
     ("年間使用量",f"={MN('C'+str(tr))}","kWh","#,##0",""),
     ("年間最大デマンド",f"={MN('D'+str(tr))}","kW","#,##0",""),
     ("年間平均電力",f"={MN('C'+str(tr))}/({MN('B'+str(tr))}*24)","kW","0.0","使用量÷年間時間"),
     ("負荷率(Load Factor)","=B8/B7","","0.0%",'=IF(B9<0.3,"低い＝断続稼働。基本料金の負担割合が大→デマンド制御の費用対効果大",IF(B9<0.5,"中位","高い＝平坦運転"))'),
     ("ピーク/谷 デマンド比",f"=MAX({MN('D4:D'+str(tr-1))})/MIN({MN('D4:D'+str(tr-1))})","倍","0.00","月別最大デマンドの変動幅"),
     ("使用量 変動係数(CV)",f"=STDEV({MN('C4:C'+str(tr-1))})/AVERAGE({MN('C4:C'+str(tr-1))})","","0.00",'=IF(B11<0.15,"季節変動小",IF(B11<0.3,"中","季節変動大→操業差の可能性"))'),
     ("実効電力単価",f"=({MN('I'+str(tr))}+{MN('J'+str(tr))})/{MN('C'+str(tr))}","円/kWh","0.0","総額÷kWh"),
     ("基本料金比率",f"={MN('J'+str(tr))}/({MN('I'+str(tr))}+{MN('J'+str(tr))})","","0.0%",'=IF(B13>0.3,"基本料金比率が高い→ピークカットが効く","従量主体")'),
     ("最大デマンド/契約電力",f"=MAX({MN('D4:D'+str(tr-1))})/MAX({MN('H4:H'+str(tr-1))})","","0.0%","1.0未満は契約に余裕。実量制で契約見直し余地"),
    ]
    r=6
    for lab,ff,u,fmt,ev in rows1:
        st(an.cell(r,1,lab),BLACK,align=LEFT); st(an.cell(r,2,ff),GREEN,None,CEN,fmt)
        st(an.cell(r,3,u),BLACK,align=CEN); st(an.cell(r,4,ev),Font(name=Fn,size=9,color="595959"),align=LEFT); r+=1
    # ② 感応度・前提リスク分析
    r+=1; an.cell(r,1,"② 感応度・前提リスク分析（最大の不確実要因＝空調比率・前提値）").font=BOLD
    s1=r+1
    an.cell(s1,1,"［基本料金削減/年：空調ピーク割合を±5pt・標準キャップ30%固定］").font=Font(name=Fn,size=9,color="C00000")
    for j,h in enumerate(["空調ピーク割合","基本料金削減/年","対 年間電気代"],1): st(an.cell(s1+1,j,h),WB_,HDR,CEN)
    for i,d in enumerate([-0.05,0.0,0.05]):
        rr=s1+2+i; sgn="+" if d>=0 else "-"; a=abs(d)
        base=f"({SC('$B$5')}*({SC('$B$6')}{sgn}{a})*{SC('$B$8')}*0.30*{SC('$B$13')}*12"+PF+")"
        st(an.cell(rr,1,f"={SC('$B$6')}{sgn}{a}"),GRN if d==0 else BLACK,None,CEN,"0%")
        st(an.cell(rr,2,"="+base),GRN if d==0 else BLACK,None,CEN,"#,##0")
        st(an.cell(rr,3,f"={base}/({SC('$B$14')}+{SC('$B$15')})"),BLACK,None,CEN,"0.0%")
    s2=s1+5
    an.cell(s2,1,"［従量削減/年：空調電力量割合(前提B6)±5pt × conv・標準30%固定］").font=Font(name=Fn,size=9,color="C00000")
    for j,h in enumerate(["電力量割合＼conv","conv 0.4","conv 0.5","conv 0.6"],1): st(an.cell(s2+1,j,h),WB_,HDR,CEN)
    for i,d in enumerate([-0.05,0.0,0.05]):
        rr=s2+2+i; sgn="+" if d>=0 else "-"; a=abs(d)
        st(an.cell(rr,1,f"='前提・制御条件'!$B$6{sgn}{a}"),GRN if d==0 else BLACK,None,CEN,"0%")
        for k,cvv in enumerate([0.4,0.5,0.6]):
            f_=f"={MN('C'+str(tr))}*('前提・制御条件'!$B$6{sgn}{a})*{SC('$B$8')}*0.30*{cvv}*{SC('$B$12')}"
            st(an.cell(rr,2+k,f_),GRN if (d==0 and cvv==0.5) else BLACK,None,CEN,"#,##0")
    an.cell(s2+5,1,"※緑＝現行前提。±5ptで削減額はほぼ比例変動。30分データ/夜間kWhで実測化を推奨。").font=Font(name=Fn,size=9,color="595959")
    # ③ シナリオ別ROI・投資判断
    r3=s2+7; an.cell(r3,1,"③ シナリオ別ROI・投資判断").font=BOLD
    n_ctrl=sum(1 for _ in units)
    st(an.cell(r3+1,1,"制御対象 台数"),BLACK,align=LEFT); st(an.cell(r3+1,2,n_ctrl),BLUE,None,CEN,"0"); an.cell(r3+1,3,"台").font=BLACK
    an.cell(r3+1,4,"投資＝初期費用合計（年間利用料・回収シート B7）").font=Font(name=Fn,size=9,color="595959")
    st(an.cell(r3+2,1,"回収年しきい値"),BLACK,align=LEFT); st(an.cell(r3+2,2,"='室外機絞り込みROI'!$G$4"),BLUE,None,CEN,"0.0"); an.cell(r3+2,3,"年").font=BLACK
    hr=r3+3
    for j,h in enumerate(["シナリオ","合計削減/年","投資額","単純回収年","5年累積純益","10年累積純益","判定"],1): st(an.cell(hr,j,h),WB_,HDR,CEN)
    INV="'年間利用料・回収'!$B$7"
    for i,(nm,col) in enumerate([("保守(20%)","B"),("標準(30%)","C"),("積極(40%)","D")]):
        rr=hr+1+i; sav=SC('$'+col+'$26')
        st(an.cell(rr,1,nm),BOLD if col=="C" else BLACK,GREENF if col=="C" else None,CEN)
        st(an.cell(rr,2,f"={sav}"),BLACK,None,CEN,"#,##0")
        st(an.cell(rr,3,f"={INV}"),BLACK,None,CEN,"#,##0")
        st(an.cell(rr,4,f"=IF({sav}=0,0,{INV}/{sav})"),BLACK,None,CEN,"0.0")
        st(an.cell(rr,5,f"={sav}*5-{INV}"),BLACK,None,CEN,"#,##0")
        st(an.cell(rr,6,f"={sav}*10-{INV}"),GRN,None,CEN,"#,##0")
        st(an.cell(rr,7,f'=IF(D{rr}<=$B${r3+2},"採用妥当","要検討")'),BOLD,None,CEN)
    # ④ データに基づく考察（自動生成）
    r4=hr+5; an.cell(r4,1,"④ データに基づく考察（自動生成）").font=BOLD
    _notes=[
     f"・負荷率 {_lf:.1%}（平均{_avg_kw:.0f}kW / 最大{max(pk):.0f}kW）。"+("低く瞬時ピークが鋭い。基本料金比率も約{:.0%}と高く、デマンド制御の費用対効果が出やすい構造。".format(_basic_ratio) if _lf<0.3 else "中〜高位で平坦運転寄り。デマンド制御の余地は相対的に小さい。"),
     f"・月別最大デマンドは{min(pk):.0f}〜{max(pk):.0f}kW（ピーク/谷 {max(pk)/min(pk):.2f}倍）。山={pkmax[0]}、谷={pkmin[0]}。",
     f"・使用量は山={kwmax[0]}({kwmax[1]:,.0f}kWh)／谷={kwmin[0]}({kwmin[1]:,.0f}kWh)、変動係数{_cv:.2f}。"+("谷が夏季にあり、操業調整（夏季減産・長期休暇）の可能性。負荷の読みは要確認。" if _valley_summer else ""),
     f"・実効電力単価 約{_eff_rate:.1f}円/kWh、基本料金比率 約{_basic_ratio:.0%}。基本料金（実量制）の削減が効きやすい。",
     f"・空調は稼働{n_ctrl}台（定格は型式推定）。総定格・制御可能比率は推定値で、小規模ゆえ絶対額は限定的。",
     "・最大の不確実要因は空調比率（前提値）。②のとおり±5ptで削減額が比例変動。30分デマンドデータ／夜間kWhでの実測検証を推奨。",
    ]
    for i,t in enumerate(_notes):
        cc=an.cell(r4+1+i,1,t); cc.font=BLACK; cc.alignment=LT
        an.merge_cells(start_row=r4+1+i,start_column=1,end_row=r4+1+i,end_column=7); an.row_dimensions[r4+1+i].height=30
    for col,w in {"A":26,"B":16,"C":10,"D":16,"E":16,"F":16,"G":12}.items(): an.column_dimensions[col].width=w
    an.row_dimensions[5].height=24
    ...
    if preserved_sheet_0:
        ...
        for i in range(12):
            ...
            st(rep.cell(r, 3, f"=ROUND(B{r}*'前提・制御条件'!$B$5*'前提・制御条件'!$B$14*{RATIO_cell}, 1)"), BLACK, LGRAY if i%2 else None, CEN, fmt="0.0")
            ...
            st(rep.cell(r, 6, f"=ROUND(E{r}*'前提・制御条件'!$B$6*'前提・制御条件'!$B$14*{RATIO_cell}*'前提・制御条件'!$B$9, 0)"), BLACK, LGRAY if i%2 else None, None, fmt="#,##0")
            ...
            st(rep.cell(r, 10, f"=ROUND(C{r}*'月次・電気料金'!G{mr}"+PF+f" + F{r}*'月次・電気料金'!F{mr}, 0)"), GRN, LGRAY if i%2 else None, None, fmt="#,##0")
